...
@Component({
  ...
  template: `
    <div class="page">
      <app-page-header title="Seksi" subtitle="Kelola data seksi" [breadcrumbs]="breadcrumbs">
...
export class SectionListComponent implements OnInit {
  private readonly api = inject(SectionsService);
  breadcrumbs: BreadcrumbItem[] = [{ label: 'Dashboard', link: '/dashboard' }, { label: 'Organisasi', link: '/organization' }, { label: 'Seksi' }];
  columns: TableColumn[] = [
    { key: 'code', label: 'Kode', width: '100px' },
    { key: 'name', label: 'Nama Seksi', sortable: true },
    { key: 'department', label: 'Departemen', format: (v) => v?.name || '-' }
  ];
  ...
  load(): void {
    ...
    this.api.sectionsGet({ page: this.currentPage() }).subscribe({
